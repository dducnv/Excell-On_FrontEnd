...
const Service = (props) => {
    ...
    const [countSpecificationEpl, setSountSpecificationEpl] = useState(null);
    ...
    const fetchDataEmployeeCount = async (id) => {
        var result = await webApi.employeeSpecification(id, startDate, endDate)
        ...
        setSountSpecificationEpl(result);
    }
    ...
    const handleAddToCart = (e) => {
        e.preventDefault();
        if (startDate != null && endDate != null && specificationID != null && employeeNumber != null) {
            if (confirmInfo == true) {
                const orderDetails = {
                    ...
                    GrandTotale: (serviceDataCart.Price * employeeNumber) * totalDay,
        ...
        return (
            <>
                <div className="mb-2">
                    ...
                    <h3 className="text-lg font-medium">From <span className="text-indigo-600 font-semibold">{startDate}</span> to <span className="text-indigo-600 font-semibold">{endDate}</span> : <span className="text-red-600 font-semibold">{countSpecificationEpl} Employees</span> </h3>
                </div>
            </>
        )
    }
    const checkEmployeeNumber = (e) => {
        CountDate();
        if (employeeNumber > countSpecificationEpl) {
            toast.error("Not enough employee for you, please re-enter.")
        }
    }
    return (
            ...
            <div>
                <div className="bg-white dark:bg-gray-600 rounded-2xl shadow-xl px-8 py-12 sm:px-12 lg:px-8">
                    ...
                    <div className="mb-12 space-y-4">
                        ...
                        <a onClick={() => addToCartModel(props.item)} className="block font-medium text-indigo-600 dark:text-indigo-400">Add To Cart</a>
                    </div>
                    ...
                    {employeeNumber &&
                        <>
                            <div className="w-full  mt-4 bg-gray-50 border py-2 px-3 border-gray-200 rounded-md mb-2">
                                ...
                                <p className="text-gray-500">Service Unit Price: <span className="font-semibold">{serviceDataCart.Price}$ / 1 Day for 1 Employee</span> </p>
                                <p className="text-gray-500">Total Price (Price x Employees): <span className="font-semibold">{serviceDataCart.Price * employeeNumber}$ / 1 Day for {employeeNumber} Employees</span> </p>
                                ...
                                <p className="text-gray-500 font-semibold">Grand Total: <span className="font-bold text-indigo-600">{(serviceDataCart.Price * employeeNumber) * totalDay}$ / {totalDay} day for {employeeNumber} Employees</span> </p>
                            </div>
